...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Observar mensajes de validación
        registerViewModel.getValidationMessage().observe(this, validationMessage -> {
    ...
    // Método que llama al ViewModel para registrar un usuario
    private void registerUser() {
        String name = etFullName.getText().toString().trim();
        String email = etEmail.getText().toString().trim();
        String password = etPassword.getText().toString().trim();
        ...
        registerViewModel.register(name, email, password, confirmPassword, phone, address);
    }
}
